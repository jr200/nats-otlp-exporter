...
if (!existsSync(baselinePath)) {
  console.log(
    `::notice title=No baseline::Baseline ${baselinePath} not found — thresholds will not be enforced. Commit the current output as the baseline (pnpm bench:update) to start comparisons.`,
  )
  process.exit(0)
}
...
function fmt(n) {
  return n == null ? '—' : n >= 1000 ? Math.round(n).toLocaleString() : n.toFixed(2)
}
function pctStr(p) {
...
let fails = 0,
  warns = 0,
  noisy = 0
const lines = ['| benchmark | baseline hz | current hz | Δ | noise |', '|---|---:|---:|---:|---:|']
for (const r of rows) {
...
if (process.env.GITHUB_ACTIONS === 'true') {
  for (const r of rows) {
    ...
    if (r.regression > FAIL_THRESHOLD) {
      console.log(
        `::error title=Benchmark regression::${r.key}: slowed ${r.regression.toFixed(1)}% (baseline ${fmt(r.base.hz)} → current ${fmt(r.cur.hz)} hz)`,
      )
    } else if (r.regression > WARN_THRESHOLD) {
      console.log(
        `::warning title=Benchmark regression::${r.key}: slowed ${r.regression.toFixed(1)}% (baseline ${fmt(r.base.hz)} → current ${fmt(r.cur.hz)} hz)`,
      )
    }
  }
